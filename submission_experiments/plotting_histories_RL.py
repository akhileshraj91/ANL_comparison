...
DIR = './experiment_data/CC_control/'
cluster = 'CC_control'
traces = {}
...
if next(os.walk(DIR))[1] == []:
    ...
    for fname in files:
        if fname.endswith("tar.xz"):
            tar = tarfile.open(DIR + '/' + fname, "r:xz")
            tar.extractall(path= DIR + '/' + fname[:-7])
            tar.close()
traces[cluster][0] = next(os.walk(DIR))[1]
...
plot_colors = ['r','g','b','k','c','m','y','maroon','goldenrod','lavender']
...
for folds in final_list:
    try:
        ...
        if flag_maximal:
            ...
            legend_name = 'PI controller' + str(color_index)
...
######################################################################################################################### RL controllers begin
experiment_type = 'controller'
DIR = './experiment_data/CC_RL_control/'
...
for folds in final_list:
    try:
        ...
        if flag_maximal:
            ...
            legend_name = 'RL controller - ' + str(color_index)
            axes2.scatter(data[cluster][trace]['rapl_sensors']['value' + str(1)].index,
                            data[cluster][trace]['rapl_sensors']['value' + str(1)], c=color,
                            s=10, label=legend_name)
            flag_maximal = 0
