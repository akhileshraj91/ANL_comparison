...
declare -r LOGDIR='./results/maximum'  # all relative paths are relative to $LOGDIR
declare -r DATADIR='./good_dyna'
declare -r OUTPUTDIR='./results/maximum'
...
do
	if [[ ${cfg} == *"dynamics"* ]]; then
		...
		do
			...
			python maximum.py max-range-config.yaml -- ones-stream-full 33554432 10000 ${cfg}
