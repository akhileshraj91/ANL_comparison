...
declare -r LOGDIR='./results/optimal'  # all relative paths are relative to $LOGDIR
declare -r DATADIR='./good_dyna'
declare -r OUTPUTDIR='./results/optimal'
...
do
	if [[ ${cfg} == *"dynamics"* ]]; then
		...
		do
			...
			python optimal.py max-range-config.yaml -- ones-stream-full 33554432 10000 ${cfg}
