...
LOGS_CONF = {
    ...
    'handlers': {
        'file': {
            ...
            'filename': f'./results/maximum/{LOGGER_NAME}.log',
            'mode': 'w',
            'level': LOGS_LEVEL,
            'formatter': 'precise',
            'filters': [],
...
def initialize_csvwriters(stack: contextlib.ExitStack):
    csvfiles = {
        msg_type: stack.enter_context(open(f'./results/maximum/dump_{msg_type}.csv', 'w'))
...
class PIController:
    def __init__(self, config, daemon, rapl_actuators):
        ...
        self.min_pcap = config['controller']['power-range'][0]
        self.max_pcap = config['controller']['power-range'][1]
